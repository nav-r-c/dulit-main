...
const registrationFormSchema = z.object({
  ...
  referral_code: z
    ...
    .refine((code) => !code || referralCodeRegex.test(code), {
      message: "Invalid referral code format. Use 01@dulit to 99@dulit.",
    }),
});
      ...
      <form style={{ marginTop: "2rem" }} onSubmit={form.onSubmit(handleSubmit)}>
        <Flex direction="column" w={{ base: "100%", lg: "50vw" }} gap="md">
          <Select
            ...
            disabled={loading}
          />
          <TextInput placeholder="e.g. John Doe" label="Name" size="lg" required {...form.getInputProps("name")} disabled={loading} />
          <TextInput placeholder="e.g. john@xmail.com" label="Email" size="lg" required {...form.getInputProps("email")} disabled={loading} />
          <NumberInput placeholder="e.g. 0123456789" label="Phone Number" size="lg" hideControls required disabled={loading} {...form.getInputProps("phone_number")} />
          <NumberInput placeholder="e.g. 18" label="Age" size="lg" hideControls required disabled={loading} {...form.getInputProps("age")} />
          <Select label="Gender" data={["Male", "Female", "Others", "Prefer not to say"]} placeholder="Choose One..." required size="lg" allowDeselect={false} rightSection={<IconChevronDown />} disabled={loading} {...form.getInputProps("gender")} />
          <TextInput placeholder="e.g. Delhi University" label="College/Profession" size="lg" required disabled={loading} {...form.getInputProps("organization")} />
          <TextInput label="Referral Code (Optional)" size="lg" disabled={loading} {...form.getInputProps("referral_code")} />
        </Flex>
